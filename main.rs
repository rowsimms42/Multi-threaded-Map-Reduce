...
fn main() {
    ...
    let p_1 = xs[0].clone();
    let p_2 = xs[1].clone();
    let th_1 = thread::spawn(move || {map_data(&p_1)});
    let th_2 = thread::spawn(move || {map_data(&p_2)});
    let res_1 = th_1.join().unwrap();
    let res_2 = th_2.join().unwrap();
    ...
    for elem in xs2{
        handles.push(thread::spawn(move || map_data(&elem)));
    }
    for h in handles{
        let q = h.join().unwrap();
        res.push(q)
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut rem = v.len() % num_partitions;
    let mut xs: Vec<Vec<usize>> = Vec::new();
    let mut count = 0;
    ...
    for _i in 0..num_partitions{
       let mut x1: Vec<usize>= Vec::new();
       if rem > 0{
           for j in count..(count+size+1){
               x1.push(v[j]);
           }
           count = count + 1;
           rem = rem-1;
       }else{
           for j in count..(count+size){
               x1.push(v[j]);
           }
       }
       xs.push(x1);
